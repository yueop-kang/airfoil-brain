...
class CVAE(nn.Module):
    def __init__(self, config, params_dict=None):
        ...
        self.beta, self.h_channels, self.h_kernel, self.h_padding, self.reduced_scale, self.activation_f, self.batch_norm, self.z_dims =\
        ...
        self.fc2 = nn.Sequential(nn.Linear(h_dim, self.z_dims))
        self.fc3 = nn.Sequential(nn.Linear(self.z_dims, h_dim))
    ...
    def forward(self, x):
        ...
        if not self.training:
            z=mu
        r = self.decode(z)
        return r, z, mu, logvar

    def loss_fn(self, r, x, mu, logvar):
        # MSE= F.mse_loss(r, x, reduction='sum') / x.shape[0]
    ...
    def training_step(self, train_batch, batch_idx):
        if batch_idx==0:
            self.train()
        x= train_batch
        r, z, mu, logvar = self(x)
        loss = self.loss_fn(r, x, mu, logvar)
        return loss
...
class CVAE_custom(nn.Module):
    def __init__(self, config, params_dict=None):
        super(CVAE_custom, self).__init__()
